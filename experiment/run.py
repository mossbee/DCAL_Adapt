...
def train(params, train_loader, val_loader, test_loader):
    ...
    # Use TwinFaceTrainer for twin face verification task
    if hasattr(params, 'data') and params.data.startswith('twin_faces'):
...
def evaluate(default_params):
    ...
    if not os.path.isfile(os.path.join(default_params.output_dir, result_name)):
        ...
        # Use TwinFaceTrainer for twin face verification task
        if hasattr(default_params, 'data') and default_params.data.startswith('twin_faces'):
            trainer = TwinFaceTrainer(model, tune_parameters, default_params)
            if not os.path.isfile(os.path.join(default_params.output_dir, 'model.pt')):
                ...
                trainer.load_weight()
            eval_metrics = trainer.eval_verification(test_loader, 'test')
            json.dump(
                {"eer": eval_metrics['eer'], "auc": eval_metrics['auc'], 
                 "tar": eval_metrics['tar'], "far": eval_metrics['far'],
                 "accuracy": eval_metrics['accuracy'], "inserted_parameters": model_grad_params_no_head,
                 'best_tune': best_tune},
