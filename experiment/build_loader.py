...
from data.dataset.twin_faces import get_twin_faces
from data.dataset.verification_dataset import get_verification_dataset
...
def get_dataset(data, params, logger):
    ...
    if data.startswith("cub"):
        ...
        if params.final_run:
            ...
            raise NotImplementedError
    elif data.startswith("twin_faces"):
        logger.info("Loading Twin Faces data ...")
        if params.final_run:
            logger.info("Loading training data (final training data for twin_faces)...")
            dataset_train = get_twin_faces(params, 'trainval_combined')
            dataset_test = get_verification_dataset(params, 'test')  # Use verification dataset for evaluation
        else:
            raise NotImplementedError
    else:
